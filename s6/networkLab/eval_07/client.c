    ...
    if (strcmp(buffer, "Authenticated\n") == 0) {
        while (1) {
            ...
            if (opt == 1) {
                ...
                printf("'q' to exit\n");
                printf("'f' for filtering using sender email id\n");
                printf("'v' for viewing the contents of a specific mail by sno\n");
                scanf("%c", &junk);
                printf("Enter: ");
                scanf("%c", &opt1);
                printf("Enter sno: ");
			...
			printf("Enter the mail Id to filter by: ");
			char id[25];
			scanf("%s", id);
			send_packet(psockfd, id, sizeof(id));
			recv_packet(psockfd, &buffer);
			printf("%s", buffer);
		}
